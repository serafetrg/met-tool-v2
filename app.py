...
API_URL = "https://dlmm-api.meteora.ag/pair/all_with_pagination?limit=100&sort_key=feetvlratio1h&order_by=desc&include_unknown=true&hide_low_tvl=1000&hide_low_apr=true&include_token_mints=EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v%2C%20So11111111111111111111111111111111111111112"
...
def format_created_at(created_at_str: str) -> str:
    try:
        ...
        return f"{days} day{'s' if days != 1 else ''} {hours} hours" if days >= 1 else f"{hours} hours"
    except Exception:
        return "N/A"
...
def process_pairs(
    ...
    for p in raw_pairs:
        ...
        for tf in ULTRA_TIMEFRAMES:
            stats = ultra.get(tf, {})
            buy_vol = float(stats.get('buyVolume', 0))
            sell_vol = float(stats.get('sellVolume', 0))
...
def display_table(pairs: List[dict], sort_field: str, reverse: bool) -> None:
    ...
    columns = [
        'Links', 'Name', 'Liquidity ($)',
        'Vol min30', 'Fee min30', 'Ratio min30',
        'Bin Step', 'Base Fee %', 'Max Fee %',
        'Created', 'MCAP', 'Organic Score', 'LP Ratio', 'Avg Vol', 'Avg Fee', 'Avg Ratio',
        'Vol 1h', 'Fee 1h', 'Ratio 1h',
        'Vol 2h', 'Fee 2h', 'Ratio 2h',
        'Vol 4h', 'Fee 4h', 'Ratio 4h',
        'Vol 12h', 'Fee 12h', 'Ratio 12h',
        'Vol 24h', 'Fee 24h', 'Ratio 24h',
        'Fees 24h ($)', 'Today Fees ($)', 'Trade Vol 24h ($)', 'Cum Trade Vol', 'Cum Fee Vol',
        'Custom Sort',
    ...
    try:
        ...
        df = df.assign(_sort_col=sort_vals).sort_values('_sort_col', ascending=not reverse).drop('_sort_col', axis=1)
    except Exception:
...
def main() -> None:
    ...
    if data and 'pairs' in data:
        ...
        with st.sidebar:
            with st.expander("🔍 Filter Pools", expanded=True):
                ...
                min_ratio_min30_input = st.number_input(
                    ...
                    value=safe_session_number("filter_input_min_ratio_min30", min_ratio_min30, max_ratio_min30, min_ratio_min30_default),
                    step=0.01,
        ...
        if not filtered_pairs:
            st.warning("No pools match your filter settings. Try lowering the minimum MCAP, pool age, 30 min volume, or min ratio.")
            return
        ...
        sort_options = [
            ...
            'Vol 12h', 'Fee 12h', 'Ratio 12h',
            'Vol 24h', 'Fee 24h', 'Ratio 24h',
            'Liquidity ($)', 'LP Ratio', 'MCAP', 'Fees 24h ($)', 'Trade Vol 24h ($)',
            'Avg Vol', 'Avg Fee', 'Avg Ratio', 'Custom Sort',
            'vol/liq 5m', 'vol/liq 1h', 'vol/liq 6h', 'vol/liq 24h'
        ]
